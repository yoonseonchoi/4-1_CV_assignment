...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embed_caption = self.positional_encoding(embed_caption)
        feat_proj = torch.reshape(self.visual_projection(features), (N, 1, -1))

        tgt_mask = torch.tril(torch.ones(T, T))
        out = self.transformer(embed_caption, feat_proj, tgt_mask)
