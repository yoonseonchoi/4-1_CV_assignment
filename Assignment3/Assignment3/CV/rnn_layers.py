...
def rnn_step_backward(dnext_h, cache):
    ...
    dout_xh = (1 - np.square(next_h)) * dnext_h      # tanh 미분
    dx = np.dot(dout_xh, Wx.T)
    dprev_h = np.dot(dout_xh, Wh.T)
    dWx = np.dot(x.T, dout_xh)
    dWh = np.dot(prev_h.T, dout_xh)
    db = np.sum(dout_xh, axis=0)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h + dh[:, t, :], dnext_c, cache[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
