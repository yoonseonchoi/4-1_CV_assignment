...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        row = torch.reshape(torch.arange(0, max_len), [-1,1])
        col = torch.arange(0, embed_dim, 2).repeat_interleave(2)
        col = torch.reshape(torch.pow(10000, -(col/embed_dim)), [1,-1])
        pe[0][:,:] = row * col
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        QK = torch.matmul(query, torch.swapaxes(key, 2, 3))
        QK /= torch.sqrt(torch.tensor(E/H))

        if attn_mask != None:
          QK = QK.masked_fill(attn_mask==0, float('-inf'))
          
        out = self.attn_drop(torch.softmax(QK, dim=-1))
        out = torch.matmul(out, value)
        out = torch.swapaxes(out, 1, 2)
        output = self.proj(torch.reshape(out, (N, S, E)))
